...
def scrape_following(bot, username, user_input):
    ...
    screenshots_folder = f'{username}_following'
    os.makedirs(screenshots_folder, exist_ok=True)

    screenshots_taken = 0

    while screenshots_taken < user_input:
        # Take a screenshot of the visible following list
        screenshot_filename = os.path.join(screenshots_folder, f'{username}_following_screenshot_{screenshots_taken + 1}.png')
        bot.save_screenshot(screenshot_filename)
        print(f"[Info] - Screenshot saved as {screenshot_filename}")
        
        # Scroll down to reveal more following
...
def scrape():
    ...
    for user in usernames:
        user = user.strip()
        scrape_following(bot, user, user_input)
